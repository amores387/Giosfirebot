module.exports = {
    ...
    onStart: async function({ send, systemData, messageData }) {
        ...
        const dailyLastUsages = getSavedData('dailyLastUsages') || {};
        const lastUse = dailyLastUsages[senderID]? new Date(dailyLastUsages[senderID]) : null;
        ...
        if (!minOffsetUsage || minOffsetUsage >= 5) {
            ...
            await serverFunc('users', 'UPDATE', { columns: ['bal'], condition: 'WHERE id=?', values: [newBal, senderID] });

            dailyLastUsages[senderID] = new Date().toISOString();
            saveData({ dailyLastUsages });

            await send(`🎁 You've claimed your reward $${amount} Go back again after 5 min`);
        } 
        
        else {
            await send(`⚠️ You've already claim your reward! Please try again after ${Math.ceil(5 - minOffsetUsage)} min`);
        };
    }
};